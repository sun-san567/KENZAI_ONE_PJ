...
<div class="ml-64 w-[calc(50%-64px)] mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-xl font-bold text-gray-800">担当者管理</h1>
        <a href="{{ route('clients.contacts.create', $client->id) }}"
    ...
    <div class="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h2 class="text-sm font-medium text-gray-700">{{ $client->name }} - 担当者一覧</h2>
                <span class="text-xs text-gray-500">{{ $contacts->total() }}件</span>
            </div>
        </div>
        ...
        <div class="overflow-hidden">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="odd:bg-white even:bg-gray-50 hover:bg-blue-50/30 transition-colors">
                        <td class="px-6 py-4">
                            ...
                            @if($contact->note)
                            <div class="text-xs text-gray-500 mt-1">{{ $contact->note }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-gray-700">{{ $contact->position }}</td>
                        <td class="px-6 py-4 text-gray-700">{{ $contact->phone }}</td>
                        <td class="px-6 py-4 text-gray-700">{{ $contact->email }}</td>
                        <td class="px-6 py-4 text-right text-sm">
                            <div class="flex justify-end space-x-2">
                                <!-- 編集ボタン -->
                                <a href="{{ route('clients.contacts.edit', [$client->id, $contact->id]) }}"
                                ...
                                <!-- 削除ボタン -->
                                <form action="{{ route('clients.contacts.destroy', [$client->id, $contact->id]) }}" method="POST" onsubmit="return confirmDelete(event)" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
            ...
            <div class="text-center py-12">
                ...
                <p class="mt-2 text-sm text-gray-500">登録されている担当者がありません</p>
                <a href="{{ route('clients.contacts.create', $client->id) }}" class="mt-3 inline-flex items-center px-3 py-1.5 text-sm text-blue-600 hover:text-blue-700">
                    + 新しい担当者を追加
                </a>
            </div>
            @endif
